...
export default function TodoList(props: AppProps) {
  return (
      ...
      <Col md={8}>
        ...
        <TodoProvider>
          <TodoContext.Consumer>
            {(context: TodoContextArray) => (
              <ContentWrapper context={context}>
                {context.todos.length > 0 ? (
                  context.todos.map((todo: TodoObject) => (
                    <Todo key={todo.id} todo={todo} history={props.history} />
                  ))
                ) : (
